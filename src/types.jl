...
The onset problem uses the conduction profile; `params.basic_state` must be
`nothing` (use `BiglobalProblem` or `TriglobalProblem` for a basic state).
...
struct OnsetProblem{T, BS}
    ...
    function OnsetProblem(params::OnsetParams{T, BS}) where {T, BS}
        params.basic_state === nothing || throw(ArgumentError(
            "OnsetProblem solves about the conduction profile and would ignore " *
            "params.basic_state; use BiglobalProblem(params, basic_state) for a " *
            "BasicState or TriglobalProblem for a BasicState3D"))
...
- `:selfconsistent`  — nonlinear steady momentum and heat balance forced by
